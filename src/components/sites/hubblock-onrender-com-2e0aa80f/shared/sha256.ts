...
/** Synchronous SHA-256 hex digest (UTF-8 input). */
export function sha256Sync(input: string): string {
  ...
  // 64-bit length: high 32 bits assumed 0 for realistic inputs
...
export function countDiffBits(a: string, b: string): number {
  let bits = 0;
...
/** Count differing hex chars between two equal-length hex strings. */
export function countDiffChars(a: string, b: string): number {
  let n = 0;
